...
public class CartServiceImpl implements CartService {
    ...
    public CartVo getCart(String userKey, String authorization) throws ExecutionException, InterruptedException {
        ...
        //后台1s的时候去慢慢修改下次看到购物车就变了；
        CompletableFuture.runAsync(()->{
            cartItems.forEach((item)->{
                //后台去更新价格；
                //远程批量接口，返回所有指定的sku的价格；
                //xxx
                //cartKey把购物车的每一个慢慢更新
                String s = redisTemplate.opsForValue().get(Constant.CACHE_SKU_INFO + item.getSkuId());
                if(StringUtils.isEmpty(s)){
                    ...
                    String s1 = rMap.get(item.getSkuId().toString());

                    CartItemVo cartItemVo = JSON.parseObject(s1, CartItemVo.class);
                    cartItemVo.setPrice(new BigDecimal(price));
    ...
    public CartVo getCartForOrder(Long id) {
        //1、登录购物车
        RMap<String, String> cart = redisson.getMap(Constant.CART_PREFIX + id);
    ...
    private CartItemVo addCartItemVo(Long skuId, Integer num, String cartKey) throws ExecutionException, InterruptedException {
        ...
        if (!StringUtils.isEmpty(item)) {
            ...
            CompletableFuture<Void> infoAsync = CompletableFuture.runAsync(() -> {
                ...
                itemVo.setNum(num);
                itemVo.setFirstPrice(data.getPrice());
            }, executor);
